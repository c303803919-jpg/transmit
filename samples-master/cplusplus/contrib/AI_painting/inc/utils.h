...
public:
    ...
    */
    ...
    static Result GetBinFileSize(std::string fileName, uint32_t& fileSize);

    static void* ReadBinFileData(std::string fileName, uint32_t& fileSize, void* binFileBufferData);

    static void GetPresenterServerParam(std::string fileName, std::string& ip, int& port);
};
