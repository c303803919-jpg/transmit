...
## yolov3_colorclassify_video样例
功能：使用yolov3模型对输入视频每一帧进行预测推理，再使用color模型对检测出来的车辆进行颜色识别并将结果打印到输出视频上。   
样例输入：原始视频MP4文件。    
样例输出：带推理结果的MP4文件。   
...
2. 模型转换。      
   ...
   | color        | 车辆颜色分类推理模型。是基于tensorflow的CNN模型。 | 请参考[https://gitee.com/ascend/ModelZoo-TensorFlow/tree/master/TensorFlow/contrib/cv/ATC_CarColor_tensorflow_AE](https://gitee.com/ascend/ModelZoo-TensorFlow/tree/master/TensorFlow/contrib/cv/ATC_CarColor_tensorflow_AE)目录中README.md下载原始模型章节下载模型文件。 |
...
1. 执行以下命令,将开发环境的 **yolov3_colorclassify_video** 目录上传到运行环境中，例如 **/home/HwHiAiUser**，并以HwHiAiUser（运行用户）登录运行环境（Host）。
