...
2. 模型转换。
    ...
    wget https://obs-9be7.obs.cn-east-2.myhuaweicloud.com/models/googlenet_imagenet_dynamic_batch/insert_op.cfg
...
```
cd $HOME/samples/cplusplus/level2_simple_inference/1_classification/googlenet_imagenet_dynamic_batch/scripts    
...
1. 执行以下命令,将开发环境的 **googlenet_imagenet_dynamic_batch** 目录上传到运行环境中，例如 **/home/HwHiAiUser**，并以HwHiAiUser（运行用户）登录运行环境（Host）。    
    ...
    cd ${HOME}/googlenet_imagenet_dynamic_batch/scripts
    ```
