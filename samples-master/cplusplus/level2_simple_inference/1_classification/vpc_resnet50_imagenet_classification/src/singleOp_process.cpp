...
Result SingleOpProcess::RunSigleOpCast()
{
    ...
    aclError ret = aclrtMalloc(&devBufferCast_, tensorSizeCast_, ACL_MEM_MALLOC_HUGE_FIRST);
    if (ret != ACL_SUCCESS) {
...
Result SingleOpProcess::RunSigleOpArgMaxV2()
{
    ...
    ret = aclrtMalloc(&devBufferArgMaxV2_, tensorSizeArgMaxV2_, ACL_MEM_MALLOC_HUGE_FIRST);
    if (ret != ACL_SUCCESS) {
