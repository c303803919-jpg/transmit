...
size_t OpRunner::GetInputSize(size_t index)
{
    if (index >= opDesc_->inputDesc.size()) {
        ERROR_LOG("index out of range. index = %zu, numInputs = %zu", index, opDesc_->inputDesc.size());
        return 0;
    }
...
std::vector<int64_t> OpRunner::GetOutputShape(size_t index)
{
    ...
    if (index >= opDesc_->outputDesc.size()) {
        ERROR_LOG("index out of range. index = %zu, numOutputs = %zu", index, opDesc_->outputDesc.size());
        return ret;
    }
...
{
    if (index >= opDesc_->inputDesc.size()) {
        ERROR_LOG("index out of range. index = %zu, numOutputs = %zu", index, opDesc_->inputDesc.size());
        return;
    }
