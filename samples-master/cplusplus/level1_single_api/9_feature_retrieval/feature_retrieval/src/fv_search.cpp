    ...
    const uint8_t *featureData, size_t dataLen)
{
    ...
    ACL_REQUIRES_SUCCESS(aclrtMalloc(&inputData, BASESHORT_D_LEN, ACL_MEM_MALLOC_HUGE_FIRST),
...
bool FVSearch1N::FeatureAdd(uint32_t id0, uint32_t id1, uint32_t offset, uint32_t featureCount)
{
    ...
    if (!FVResource::GetInstance().IsDevice()) {
        // copy data to device
        ret = aclrtMalloc(&inputData, featureLen * featureCount, ACL_MEM_MALLOC_HUGE_FIRST);
        if (ret != ACL_SUCCESS) {
...
bool FVSearch1N::FeatureSearchPreProc(uint32_t queryCnt, uint32_t topK)
{
    ...
    void *devPtr = nullptr;
    ACL_REQUIRES_SUCCESS(aclrtMalloc(&devPtr, tableDataLen, ACL_MEM_MALLOC_HUGE_FIRST),
                         ERROR_LOG("aclrtMemcpy failed!!!"), return false);
    ...
    ACL_REQUIRES_SUCCESS(aclrtMalloc(&devPtr, resultNumDataLen, ACL_MEM_MALLOC_HUGE_FIRST),
                         ERROR_LOG("aclrtMemcpy failed!!!"), return false);
    resultNumDev_.reset(devPtr, [](void *p) {(void)aclrtFree(p);});

    ACL_REQUIRES_SUCCESS(aclrtMalloc(&devPtr, dataLen, ACL_MEM_MALLOC_HUGE_FIRST),
                         ERROR_LOG("aclrtMemcpy failed!!!"), return false);
