...
// vdec configuration parameter
const char *VDEC_FILE_NAME = "dvpp_vdec_h264_1frame_bp_51_1920x1080.h264";
const uint32_t STREAM_WIDTH = 1920;   // Input stream width
const uint32_t STREAM_HEIGHT = 1080;  // Input stream height
...
const int32_t OUT_BUFFER_CNT = 20;

// vpc configuration parameter
...
// Create threads according to actual needs
...
int32_t vdec_outbuf_alloc(void)
{
    ...
    for (uint32_t i = 0; i < OUT_BUFFER_CNT; i++) {
        // Alloc out buffer
        int32_t ret =
...
int32_t handle_vpc_output_data(hi_vpc_pic_info &outputPic, uint32_t dstBufferSize, const char outputFileName[])
{
    ...
    static uint32_t writeFileCnt = 0;

    int32_t ret = snprintf(saveFileName, sizeof(saveFileName), "%u_%s", writeFileCnt, outputFileName);
    if (ret <= 0) {
        printf("[%s][%d] snprintf failed, ret = %d!\n", __FUNCTION__, __LINE__, ret);
        return HI_FAILURE;
    }
    ret = HI_SUCCESS;
    ...
    if (g_run_mode == ACL_HOST) {
        ...
        ret = aclrtMallocHost(&outputPicHost.picture_address, outputPic.picture_buffer_size);
        if (ret != ACL_SUCCESS) {
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    ret = vpc_create();
    if (ret != HI_SUCCESS) {
        printf("[%s][%d] VpcStart failed!\n", __FUNCTION__, __LINE__);
        vdec_destroy();
        hi_dvpp_deinit();
        return 0;
    }

    ret = vdec_outbuf_alloc();
    if (ret != HI_SUCCESS) {
        printf("[%s][%d] Alloc Outbuf failed!\n", __FUNCTION__, __LINE__);
        vpc_destroy();
        vdec_destroy();
        hi_dvpp_deinit();
        return 0;
