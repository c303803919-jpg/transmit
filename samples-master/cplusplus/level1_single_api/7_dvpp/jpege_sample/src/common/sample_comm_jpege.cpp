...
void delay_exec(uint64_t execTime, int delaySeconds)
{
    ...
    // Call creatDvppApi after a delay of 10 seconds
    ...
    hi_data_bit_width inputBitWidth, hi_compress_mode cmpMode, uint32_t align, HiSampleCalConfig *calConfig)
{
    ...
    if (cmpMode == HI_COMPRESS_MODE_NONE) {
        ...
        if ((pixelFormat >= HI_PIXEL_FORMAT_YUYV_PACKED_422) && (pixelFormat <= HI_PIXEL_FORMAT_VYUY_PACKED_422)) {
            mainStride = ALIGN_UP(((inputWidth * bitWidth + 7) >> 3) * 2, align); // Align after doubling the width
        }
...
void send_frame_for_dc_performance(FILE *fpYuv, HiSampleJpegeSendFramePara* jpegeSendPara)
{
    ...
    delay = 30; // unit：s
    delay_exec(g_start_send_time, delay); // All threads are waiting for concurrency
...
* Description: Create a channel and receive input images
* input:       vencChn:channel id，size:channel size, supportDc: only support DC, level: Encoding quality
* output:      Create channel and receive input image successfully
* return:      HI_SUCCESS: success, error code: failed
* others:      none
*/
...
void* jpege_snap_process(void* p)
{
    ...
    // 3: Notify the user to retrieve the output stream
    while (jpegePara->threadStart == HI_TRUE) {
...
* Description: Stop obtaining output stream and destroy thread
* input:       cnt: number of channels
* output:      Stop obtaining output stream and destroy thread successfully
...
* Description: Stop sending input data and destroy thread
* input:       cnt:number of channels
* output:      Stop sending input data and destroy thread successfully
* return:      HI_SUCCESS：success
* others:      none
*/
...
* Description: Waiting for encoding to complete
* input:       chnNumStart:Starting channel id, chnNum:number of channels
* output:      none
* return:      none
* others:      none
*/
