...
int32_t g_display_rate = 30;
bool g_set_display_rate = false;
int32_t g_max_qp = 51;
int32_t g_min_qp = 20;
...
inline void init_h264_params(VencParam* encParam, uint32_t chnId)
{
    ...
    encParam->userSetDisplayRate = g_set_display_rate;
    encParam->displayRate = g_display_rate;
    ...
    while (1) {
        ...
        switch (c) {
            ...
            case 'f':
                ...
                break;
            case 'D':
                g_display_rate  = atoi(optarg);
                g_set_display_rate = true;
                break;
            case 'i':
...
hi_void venc_print_usage()
{
    ...
    HMEV_HISDK_PRT(INFO, "--BitRate(-b): bit rate for rate control");
    HMEV_HISDK_PRT(INFO, "--FrameRate(-f): frame rate for rate control, default 30fps");
    HMEV_HISDK_PRT(INFO, "--DisplayRate(-D): display frame rate for video player, default is unset");
    HMEV_HISDK_PRT(INFO, "--ChnNum(-n): encode channel number");
