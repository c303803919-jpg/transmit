...
// Wait for delay seconds based on the time before starting the channel
// To prevent significant differences in performance data between channels
...
// Save the YUV_SemiPlanar format pixel content decoded by JPEG
...
// Save the RGB format pixel content decoded by JPEG
...
// Save the pixel content decoded by JPEG
...
// JPEGD compatibility testing thread function: 
// Read all. jpg files in the folder and send them to VDEC
...
// JPEGD performance testing thread: repeatedly decode an image and calculate fps
void *jpegd_send_stream_performance(void *pArgs)
{
    ...
    // Wait for g_delay_time seconds before sending the stream,
    // ensuring that all channels start working at the same time
...
void *jpegd_send_stream(void *pArgs)
{
    ...
    while (1) {
        ...
        if (g_run_mode == ACL_HOST) {
            ...
            stStream.addr  = (uint8_t *)&fileData[0]; // Host: hi_mpi_dvpp_get_image_info use host memory
        } else {
            stStream.addr = (uint8_t *)pu8Buf; // Device: hi_mpi_dvpp_get_image_info use device memory
        }
        ...
        // hi_mpi_vdec_send_stream interface use device memory
...
void *jpegd_get_pic_performance(void *pArgs)
{
    ...
    while (1) {
        ...
        if (s32Ret == HI_SUCCESS) {
            ...
            // Output pixel memory back to the queue, performance testing does not repeatedly request memory
            (void)pthread_mutex_lock(&g_out_buffer_pool_lock[pstJpegdThreadParam->s32ChnId]);
