...
│   ├── HighPerfMatMulLeakyReluCustom             // MatmulLeakyReluCustom算子工程。
│   └── matmul_leakyrelu_custom.json      // MatmulLeakyReluCustom算子的原型定义json文件。
```

## 算子工程介绍
其中，算子工程目录HighPerfMatMulLeakyReluCustom包含算子实现的模板文件、编译脚本等，如下所示：
```
├── HighPerfMatMulLeakyReluCustom           // MatmulLeakyRelu自定义算子工程。
│   ├── cmake
...
编译自定义算子工程，构建生成自定义算子包。
  - 执行如下命令，切换到算子工程MatmulLeakyReluCustom目录。   
    ...
    ```bash
    cd ${git_clone_path}/samples/operator_contrib/MatmulLeakyReluCustom/HighPerfMatMulLeakyReluCustom
    ```
...
| 时间       | 更新事项                |
| ---------- | ----------------------- |
| 2025/03/25 | 新增AclNNInvocation样例 |
