## 目录结构介绍
```
├── AclNNInvocation             //通过aclnn调用的方式调用GeluCustom算子
│   ├── inc                     // 头文件目录
...
自定义算子编译部署后，会自动生成单算子API，可以直接在应用程序中调用。算子API的形式一般定义为“两段式接口”，形如：
   ...
   aclnnStatus aclnnGeluGetWorkspaceSize(const aclTensor *x, const aclTensor *y, const alcTensor *out, uint64_t workspaceSize, aclOpExecutor **executor);
   // 执行算子
   aclnnStatus aclnnGeluCustom(void *workspace, int64_t workspaceSize, aclOpExecutor **executor, aclrtStream stream);
   ```
...
### 2. aclnn调用样例运行
  ...
  - 进入到样例目录
    ...
    ```bash
    cd $HOME/samples/operator_contrib/GeluSample/FrameworkLaunch/AclNNInvocation
    ```
  - 样例执行
