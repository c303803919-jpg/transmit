## Overview
This example, based on the GELU operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Gelu operator using the framework
│   ├── AclNNInvocation       // Invokes the Gelu operator using the aclnn method
│   ├── GELU             // Gelu operator project
│   └── gelu.json        // Prototype definition json file for the GELU operator
``` 

## Operator Project Introduction
The operator project directory Gelu contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── GELU               // Gelu custom operator project
│   ├── cmake
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Gelu operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/GeluSample/FrameworkLaunch/GELU
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Gelu operator project directory to compile the operator project.
...
- [Invoke the Gelu Operator Project using aclnn](./AclNNInvocation/README.en.md)
