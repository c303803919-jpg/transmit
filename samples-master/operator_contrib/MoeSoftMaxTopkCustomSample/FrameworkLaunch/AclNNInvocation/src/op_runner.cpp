...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnMoeSoftMaxTopkGetWorkspaceSize(inputTensor_[0], opDesc_->k, outputTensor_[0], outputTensor_[1],
	...
	INFO_LOG("Execute aclnnMoeSoftMaxTopkGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnMoeSoftMaxTopk(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnMoeSoftMaxTopk success");
