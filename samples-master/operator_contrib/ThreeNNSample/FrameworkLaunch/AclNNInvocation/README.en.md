...
│   │   └── verify_result_int.py    // Ground truth comparison file
│   ├── src
...
After the custom operator is compiled and deployed, the single operator API is automatically generated and can be directly called in the application. The operator API is generally defined as a "two-stage interface", such as:
   ```cpp    
   aclnnStatus aclnnThreeNNGetWorkspaceSize(const aclTensor *xyz1, const aclTensor *xyz2, const aclTensor *distOut, 
                                                const aclTensor *indicesOut, uint64_t workspaceSize, aclOpExecutor **executor);
   aclnnStatus aclnnThreeNN(void *workspace, int64_t workspaceSize, aclOpExecutor **executor, aclrtStream stream);
   ```
Where aclnnThreeNNGetWorkspaceSize is the first stage interface, mainly used to calculate how much workspace memory is needed during this API call. After obtaining the required workspace size for this API calculation, allocate Device-side memory according to the workspaceSize, and then call the second stage interface aclnnTriu to perform the calculation.
...
### 2. Running the aclnn Example
  ...
  - Enter the example directory
    ...
    ```bash
    cd ${git_clone_path}/samples/operator_contrib/ThreeNNSample/FrameworkLaunch/AclNNInvocation
    ```
  - Example execution    
