...
function main {
    ...
    # 5. 比较真值文件
    cd $CURRENT_DIR
    ret_dist=`python3 scripts/verify_result.py output/output_dist.bin output/golden_dist.bin`
    ret_idx=`python3 scripts/verify_result_int.py output/output_idx.bin output/golden_idx.bin`
    echo "verify dist $ret_dist"
    echo "verify idx $ret_idx"
    if [ "x$ret_dist" == "xtest pass" ]  && [ "x$ret_idx" == "xtest pass" ]; then
        echo ""
        echo "#####################################"
