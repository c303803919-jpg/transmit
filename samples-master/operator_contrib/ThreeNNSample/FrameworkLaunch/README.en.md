## Overview
This example, based on the ThreeNN operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the ThreeNN operator using the framework
│   ├── AclNNInvocation       // Invokes the ThreeNN operator using the aclnn method
│   ├── ThreeNN             // ThreeNN operator project
│   └── ThreeNN.json        // Prototype definition json file for the ThreeNN operator
``` 

## Operator Project Introduction
The operator project directory ThreeNN contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── ThreeNN               // ThreeNN custom operator project
│   ├── cmake
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the ThreeNN operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/ThreeNNSample/FrameworkLaunch/ThreeNN
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the ThreeNN operator project directory to compile the operator project.
...
- [Invoke the ThreeNN Operator Project using aclnn](./AclNNInvocation/README.en.md)
