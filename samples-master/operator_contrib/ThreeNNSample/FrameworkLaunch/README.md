## 概述
本样例基于ThreeNN算子工程，介绍了单算子调用的调用方式。
## 目录结构介绍
``` 
├── FrameworkLaunch           //使用框架调用的方式调用ThreeNN算子
│   ├── AclNNInvocation       // 通过aclnn调用的方式调用ThreeNN算子
│   ├── ThreeNN             // ThreeNN算子工程
│   └── ThreeNN.json        // ThreeNN算子的原型定义json文件
``` 
## 算子工程介绍
算子工程目录ThreeNN包含算子实现的模板文件、编译脚本等，如下所示:
``` 
├── ThreeNN                 //ThreeNN自定义算子工程
│   ├── cmake
...
### 2.&nbsp;编译算子工程<a name="operatorcompile"></a>
  ...
  - 修改CMakePresets.json中ASCEND_CANN_PACKAGE_PATH为CANN软件包安装后的实际路径。
    ...
    ```
  - 在算子工程ThreeNN目录下执行如下命令，进行算子工程编译。
...
### 4.&nbsp;配置环境变量
  ...
  - 指定路径install_path，安装CANN软件包
    ...
    ```
### 5.&nbsp;调用执行算子工程
- [aclnn调用ThreeNN算子工程](./AclNNInvocation/README.md)
