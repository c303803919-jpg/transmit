## ThreeNN自定义算子样例说明 
本样例通过Ascend C编程语言实现了ThreeNN算子，并按照不同的算子调用方式分别给出了对应的端到端实现。
- [FrameworkLaunch](./FrameworkLaunch)：使用框架调用ThreeNN自定义算子。  
...
<table>
    ...
    <tr>
        <!-- 列的方向占据1个cell -->
        <td rowspan='1'><a href="./FrameworkLaunch"> FrameworkLaunch</td><td><a href="./FrameworkLaunch/AclNNInvocation"> AclNNInvocation</td><td>通过aclnn调用的方式调用ThreeNN算子。</td>
    </tr>
</table>

## 算子描述
对于"目标点集"中每个点，找到"比较点集"中距离其最近的3个点的距离、坐标，返回的三个点会按照距离平方升序。
目标点集：包含需要查找最近点的所有目标点。
比较点集：包含用来计算距离并查找最近点的所有比较点。

## 算子规格描述
<table>  
<tr><th align="center">算子类型(OpType)</th><th colspan="5" align="center">ThreeNN</th></tr>  
...
<tr><td rowspan="1" align="center">核函数名</td><td colspan="5" align="center">ThreeNN</td></tr>  
</table>
...
## 目录结构介绍
其中，算子工程目录ThreeNN包含算子实现的模板文件、编译脚本等，如下所示:
```
└── FrameworkLaunch        // 使用框架调用的方式调用ThreeNN自定义算子工程。
```
