...
{
    // define operator
    std::vector<int64_t> shape {3, 3,3,32};
    std::vector<int64_t> shape_dim { 1 };
    aclDataType dataType = ACL_FLOAT;
    aclDataType dimType = ACL_INT32;
    aclFormat format = ACL_FORMAT_ND;
    OperatorDesc opDesc;
    opDesc.exclusive = false;
    opDesc.reverse = false;
    opDesc.AddInputTensorDesc(dataType, shape.size(), shape.data(), format);
    opDesc.AddInputTensorDesc(dimType, shape_dim.size(), shape_dim.data(), format);
...
bool SetInputData(OpRunner &runner)
{
    ...
    ReadFile("../input/input_axis.bin", fileSize, runner.GetInputBuffer<void>(1), runner.GetInputSize(1));
