## Overview
This example, based on the Cumsum operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Cumsum operator using the framework
│   ├── AclNNInvocation       // Invokes the Cumsum operator using the aclnn method
│   ├── Cumsum              // Cumsum operator project
│   └── Cumsum.json        // Prototype definition json file for the Cumsum operator
``` 

## Operator Project Introduction
The operator project directory Cumsum contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── Cumsum               // Cumsum custom operator project
│   ├── cmake
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Cumsum operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/CumsumSample/FrameworkLaunch/Cumsum
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Cumsum operator project directory to compile the operator project.
...
- [Invoke the Cumsum Operator Project using aclnn](./AclNNInvocation/README.en.md)
