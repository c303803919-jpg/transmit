## Overview
This example, based on the Cross operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Cross operator using the framework
│   ├── AclNNInvocation       // Invokes the Cross operator using the aclnn method
│   ├── Cross             // Cross operator project
│   └── Cross.json        // Prototype definition json file for the Cross operator
``` 

## Operator Project Introduction
The operator project directory Cross contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── Cross               // Cross custom operator project
│   ├── cmake
...
``` 
The CANN software package provides the project creation tool msopgen, and the Cross operator project can be automatically created through Cross.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Cross operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/CrossSample/FrameworkLaunch/Cross
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Cross operator project directory to compile the operator project.
...
- [Invoke the Cross Operator Project using aclnn](./AclNNInvocation/README.en.md)
