...
## 概述
`MatmulCustomMultiCore` 是一个自定义的矩阵乘法操作，旨在多核处理器上高效地执行矩阵乘法。该操作通过分块和并行计算的方式，优化了矩阵乘法的性能。
...
## 性能优化
- **L1数据加载优化**：尽可能占满L1，计算出合理的depthA1、depthB1、stepKa、stepKb。如果屏蔽该部分优化，Cube利用率会从84%降低到78%，算子耗时从9964us 增长到10986us
- **L2数据缓存优化**：满足L2数据不置换的情况下通过遍历得到最优切分策略。 如果屏蔽该部分优化，Cube利用率会从84%降低到25%，算子耗时从9964us 增长到34375us
- **错位绑核**：通过错位绑核的方式规避同地址访问。如果屏蔽该部分优化，Cube利用率会从84%降低到82%，算子耗时从9964us 增长到10181us
- **设置硬件配置**：
   - 使能HF32模式 ，L0A/L0B中的FP32数据将在矩阵乘法之前被CUBE舍入为HF32。 屏蔽该优化，Cube利用率会从84%上升至99%，单算子耗时会从9964us增长到16423us。
