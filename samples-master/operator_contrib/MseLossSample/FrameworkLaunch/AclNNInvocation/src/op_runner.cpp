...
bool OpRunner::RunOp()
{
    ...
    string reduction;
	auto ret = aclnnMseLossGetWorkspaceSize(inputTensor_[0], inputTensor_[1], opDesc_->reduction, outputTensor_[0],
    ...
    ret = aclnnMseLoss(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
