...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnOnesGetWorkspaceSize(opDesc_->h, opDesc_->w, outputTensor_[0],
	...
	INFO_LOG("Execute aclnnOnesGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnOnes(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnOnes success");
