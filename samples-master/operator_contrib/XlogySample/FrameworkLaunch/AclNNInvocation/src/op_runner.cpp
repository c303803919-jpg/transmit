...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnXlogyGetWorkspaceSize(inputTensor_[0], inputTensor_[1], outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnXlogy(workspace, workspaceSize, handle, stream);
