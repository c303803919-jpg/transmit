## Overview
This example, based on the AsStrided operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the AsStrided operator using the framework
│   ├── AclNNInvocation       // Invokes the AsStrided operator using the aclnn method
│   ├── AsStrided             // AsStrided operator project
│   └── AsStrided.json        // Prototype definition json file for the AsStrided operator
``` 

## Operator Project Introduction
The operator project directory AsStrided contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── AsStrided               // AsStrided custom operator project
│   ├── cmake
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the AsStrided operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/AsStridedSample/FrameworkLaunch/AsStrided
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the AsStrided operator project directory to compile the operator project.
...
- [Invoke the AsStrided Operator Project using aclnn](./AclNNInvocation/README.en.md)
