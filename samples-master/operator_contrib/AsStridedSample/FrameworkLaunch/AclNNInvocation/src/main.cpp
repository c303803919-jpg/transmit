...
{
    // define operator
    std::vector<int64_t> shape_x {50,400};
    std::vector<int64_t> shape_other_input {2};
    std::vector<int64_t> shape_y {64,64};
    aclDataType dataType = ACL_FLOAT;
    aclDataType dataTypeOther = ACL_INT32;
    aclFormat format = ACL_FORMAT_ND;
    OperatorDesc opDesc;
    opDesc.AddInputTensorDesc(dataType, shape_x.size(), shape_x.data(), format);
    opDesc.AddInputTensorDesc(dataTypeOther, shape_other_input.size(), shape_other_input.data(), format);
    opDesc.AddInputTensorDesc(dataTypeOther, shape_other_input.size(), shape_other_input.data(), format);
    opDesc.AddInputTensorDesc(dataTypeOther, shape_other_input.size(), shape_other_input.data(), format);
...
bool SetInputData(OpRunner &runner)
{
    ...
    ReadFile("../input/input_size.bin", fileSize, runner.GetInputBuffer<void>(1), runner.GetInputSize(1));
    ReadFile("../input/input_stride.bin", fileSize, runner.GetInputBuffer<void>(2), runner.GetInputSize(2)); // 读取输入步幅数据
    ReadFile("../input/input_storage_offset.bin", fileSize, runner.GetInputBuffer<void>(3), runner.GetInputSize(3)); // 读取存储偏移文件
