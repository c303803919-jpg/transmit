...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnAsStridedGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], inputTensor_[3], outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnAsStrided(workspace, workspaceSize, handle, stream);
