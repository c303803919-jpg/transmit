...
bool OpRunner::RunOp()
{
    ...
    string reduction;
	auto ret = aclnnScatterMaxGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2],opDesc_->use_locking,
    ...
    ret = aclnnScatterMax(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
