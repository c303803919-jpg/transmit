## Triu自定义算子样例说明 
本样例通过Ascend C编程语言实现了Triu算子，并按照不同的算子调用方式分别给出了对应的端到端实现。
- [FrameworkLaunch](./FrameworkLaunch)：使用框架调用Triu自定义算子。  
...
<table>
    ...
    <tr>
        <!-- 列的方向占据1个cell -->
        <td rowspan='1'><a href="./FrameworkLaunch"> FrameworkLaunch</td><td><a href="./FrameworkLaunch/AclNNInvocation"> AclNNInvocation</td><td>通过aclnn调用的方式调用Triu算子。</td>
    </tr>
</table>

## 算子描述
Triu算子是PyTorch中的一种常见矩阵构造函数。Triu函数默认情况下返回一个矩阵主对角线以上的上三角矩阵，其它元素全部为0。主对角线的偏移由可选参数diagonal决定，其缺省值为0。diagonal为正值时，主对角线向上偏移。当输入是一个多维张量时，其最后两个维度构成矩阵，Triu以迭代的方式处理多维张量中的每个矩阵，最终返回对应的上三角矩阵构成的多维张量。返回的多维张量与输入张量维度保持一致。

## 算子规格描述
<table>
<tr><th align="center">算子类型(OpType)</th><th colspan="5" align="center">Triu</th></tr>
...
<tr><td rowspan="1" align="center">核函数名</td><td colspan="5" align="center">triu</td></td></tr>
</table>
...
## 目录结构介绍
```
└── FrameworkLaunch    //使用框架调用的方式调用Triu自定义算子工程。
```
## 环境要求
