...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnTriuGetWorkspaceSize(inputTensor_[0], opDesc_->diagonal, outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnTriu(workspace, workspaceSize, handle, stream);
