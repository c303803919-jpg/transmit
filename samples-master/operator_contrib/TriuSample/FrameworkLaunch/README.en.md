## Overview
This example, based on the Triu operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Triu operator using the framework
│   ├── AclNNInvocation       // Invokes the Triu operator using the aclnn method
│   ├── Triu             // Triu operator project
│   └── Triu.json        // Prototype definition json file for the Triu operator
``` 

## Operator Project Introduction
The operator project directory Triu contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── Triu               // Triu custom operator project
│   ├── cmake
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Triu operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/TriuSample/FrameworkLaunch/Triu
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Triu operator project directory to compile the operator project.
...
- [Invoke the Triu Operator Project using aclnn](./AclNNInvocation/README.en.md)
