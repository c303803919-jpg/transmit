## Overview
This example, based on the InstanceNorm operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the InstanceNorm operator using the framework
│   ├── AclNNInvocation       // Invokes the InstanceNorm operator using the aclnn method
│   ├── InstanceNorm             // InstanceNorm operator project
│   └── InstanceNorm.json        // Prototype definition json file for the InstanceNorm operator
``` 

## Operator Project Introduction
The operator project directory InstanceNorm contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── InstanceNorm               // InstanceNorm custom operator project
│   ├── cmake
...
``` 
The CANN software package provides the project creation tool msopgen, and the InstanceNorm operator project can be automatically created through InstanceNorm.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the InstanceNorm operator project directory.
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the InstanceNorm operator project directory to compile the operator project.
...
- [Invoke the InstanceNorm Operator Project using aclnn](./AclNNInvocation/README.en.md)
