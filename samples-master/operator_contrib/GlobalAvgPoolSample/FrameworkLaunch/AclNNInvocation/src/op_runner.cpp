...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnGlobalAvgPoolGetWorkspaceSize(inputTensor_[0], outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnGlobalAvgPool(workspace, workspaceSize, handle, stream);
