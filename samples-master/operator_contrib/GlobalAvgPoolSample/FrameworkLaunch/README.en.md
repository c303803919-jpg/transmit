## Overview
This example, based on the GlobalAvgPool operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the GlobalAvgPool operator using the framework
│   ├── AclNNInvocation       // Invokes the GlobalAvgPool operator using the aclnn method
│   ├── GlobalAvgPool             // GlobalAvgPool operator project
│   └── GlobalAvgPool.json        // Prototype definition json file for the GlobalAvgPool operator
```

## Operator Project Introduction
The operator project directory GlobalAvgPool contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── GlobalAvgPool               // GlobalAvgPool custom operator project
│   ├── cmake
...
```
The CANN software package provides the project creation tool msopgen, and the GlobalAvgPool operator project can be automatically created through GlobalAvgPool.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the GlobalAvgPool operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/GlobalAvgPoolSample/FrameworkLaunch/GlobalAvgPool
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the GlobalAvgPool operator project directory to compile the operator project.
...
- [Invoke the GlobalAvgPool Operator Project using aclnn](./AclNNInvocation/README.en.md)
