...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnGreaterEqualGetWorkspaceSize(inputTensor_[0], inputTensor_[1], outputTensor_[0],
	...
	INFO_LOG("Execute aclnnGreaterEqualGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnGreaterEqual(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
