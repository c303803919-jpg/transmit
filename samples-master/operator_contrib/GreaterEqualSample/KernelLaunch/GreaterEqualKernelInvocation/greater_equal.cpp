...
__aicore__ inline void DataCopyPadCustom_GM2UB(const AscendC::LocalTensor<typeT>& dstLocal, const AscendC::GlobalTensor<typeT>& srcGlobal, const uint32_t calCount)
{
...
template <typename typeT>
__aicore__ inline void DataCopyPadCustom_UB2GM(const AscendC::GlobalTensor<typeT>& dstGlobal, const AscendC::LocalTensor<typeT>& srcLocal, const uint32_t calCount)
{
...
extern "C" __global__ __aicore__ void greater_equal(GM_ADDR x1, GM_ADDR x2, GM_ADDR y, GM_ADDR workspace, SplitTilingData tiling)
{
  KernelGreaterEqual<T> op;
