## Overview
This example, based on the Dawsn operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Dawsn operator using the framework
│   ├── AclNNInvocation       // Invokes the Dawsn operator using the aclnn method
│   ├── Dawsn             // Dawsn operator project
│   └── Dawsn.json        // Prototype definition json file for the Dawsn operator
```

## Operator Project Introduction
The operator project directory Dawsn contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── Dawsn               // Dawsn custom operator project
│   ├── cmake
...
```
The CANN software package provides the project creation tool msopgen, and the Dawsn operator project can be automatically created through Dawsn.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
Before compiling this example, please refer to [Preparation: Obtain the Example Code](../README.en.md#codeready) to complete the source package acquisition.
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Dawsn operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/DawsnSample/FrameworkLaunch/Dawsn
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Dawsn operator project directory to compile the operator project.
...
- [Invoke the Dawsn Operator Project using aclnn](./AclNNInvocation/README.en.md)

## Update Log
  | Date | Update Items |
|----|------|
| 2024/10/30 | Create the README document |