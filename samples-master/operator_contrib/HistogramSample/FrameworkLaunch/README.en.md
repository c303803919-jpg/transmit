## Overview
This example, based on the Histogram operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Histogram operator using the framework
│   ├── AclNNInvocation       // Invokes the Histogram operator using the aclnn method
│   ├── Histogram            // Histogram operator project
│   └── Histogram.json        // Prototype definition json file for the Histogram operator
``` 

## Operator Project Introduction
The operator project directory Histogram contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── Histogram               // Histogram custom operator project
│   ├── cmake
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Histogram operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/HistogramSample/FrameworkLaunch/Histogram
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Histogram operator project directory to compile the operator project.
...
- [Invoke the Histogram Operator Project using aclnn](./AclNNInvocation/README.en.md)
