## Overview
This example is based on the BallQuery operator project and introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invoking the BallQuery operator using the framework
│   ├── AclNNInvocation       // Invoking the BallQuery operator using the aclnn method
│   ├── BallQuery               // BallQuery operator project
│   └── ball_query.json          // Prototype definition json file for the BallQuery operator
``` 
## Operator Project Introduction
The operator project directory BallQuery contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── BallQuery               // BallQuery custom operator project
│   ├── cmake
...
``` 
The CANN software package provides the project creation tool msopgen. The BallQuery operator project can be automatically created via BallQuery.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
- Execute the following command to switch to the BallQuery operator project directory.

  ```bash
  cd ${git_clone_path}/samples/operator_contrib/BallQuerySample/FrameworkLaunch/BallQuery
  ```
...
- Execute the following command in the BallQuery operator project directory to compile the operator project.
...
- [Invoking the BallQuery Operator Project using aclnn](./AclNNInvocation/README.en.md)
