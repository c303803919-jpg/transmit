...
*/
#include "op_runner.h"
#include "aclnn_pre_layer_norm_custom.h"
#include <limits>
#include <cassert>
...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnPreLayerNormCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2],inputTensor_[3],
                                              opDesc_->epsilon, outputTensor_[0],
	...
	INFO_LOG("Execute aclnnPreLayerNormCustomGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnPreLayerNormCustom(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnPreLayerNormCustom success");
