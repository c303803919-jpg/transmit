## Overview
This example, based on the PreLayerNormCustom operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the PreLayerNorm operator using the framework
│   ├── AclNNInvocation       // Invokes the PreLayerNormCustom operator using the aclnn method
│   ├── PreLayerNormCustom    // PreLayerNormCustom operator project
│   └── pre_layer_norm.json   // Prototype definition json file for the PreLayerNormCustom operator
``` 

## Operator Project Introduction
The operator project directory PreLayerNormCustom contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── PreLayerNormCustom      // PreLayerNorm custom operator project
│   ├── cmake
...
``` 
The CANN software package provides the project creation tool msopgen, and the PreLayerNormCustom operator project can be automatically created through pre_layer_norm.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the PreLayerNormCustom operator project directory.
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the PreLayerNormCustom operator project directory to compile the operator project.
...
- [Invoke the PreLayerNormCustom Operator Project using aclnn](./AclNNInvocation/README.en.md)

## Update Log
  | Date | Update Items |
|----|------|
| 2024/07/02 | New version of readme updated |
