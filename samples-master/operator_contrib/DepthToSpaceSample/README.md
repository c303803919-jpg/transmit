## DepthToSpace自定义算子样例说明 
本样例通过Ascend C编程语言实现了DepthToSpace算子，并给出了FrameworkLaunch调用的端到端实现。
- [FrameworkLaunch](./FrameworkLaunch)：使用框架调用DepthToSpace自定义算子。  
...
<table>
    ...
    <tr>
        <!-- 列的方向占据1个cell -->
        <td rowspan='1'><a href="./FrameworkLaunch"> FrameworkLaunch</td><td><a href="./FrameworkLaunch/AclNNInvocation"> AclNNInvocation</td><td>通过aclnn调用的方式调用DepthToSpace算子。</td>
    </tr>
</table>

## 算子描述
DepthToSpace算子将数据从深度重新排列为空间数据块，更具体地说，此操作输出输入张量的副本，其中深度(C)维度的值在空间块中移动到高度(H)和宽度(W)维度。
## 算子规格描述

<table>  
<tr><th align="center">算子类型(OpType)</th><th colspan="5" align="center">DepthToSpace</th></tr> 
...
<tr><td rowspan="1" align="center">核函数名</td><td colspan="5" align="center">depth_to_sapce</td></tr>  
</table>
...
## 目录结构介绍
```
└── FrameworkLaunch    //使用框架调用的方式调用DepthToSpace自定义算子工程。
```
## 环境要求
