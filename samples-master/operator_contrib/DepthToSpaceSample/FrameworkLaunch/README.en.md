## Overview
This example describes how to invoke a single operator based on the DepthToSpace operator project.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the DepthToSpace operator using the framework
│   ├── AclNNInvocation       // Invokes the DepthToSpace operator using the aclnn method
│   ├── DepthToSpace             // DepthToSpace operator project
│   └── DepthToSpace.json        // Prototype definition json file for the DepthToSpace operator
``` 

## Operator Project Introduction
The operator project directory DepthToSpace contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── DepthToSpace               // DepthToSpace custom operator project
│   ├── cmake
...
``` 
The CANN software package provides the project creation tool msopgen, and the DepthToSpace operator project can be automatically created through DepthToSpace.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
- Execute the following command to switch to the DepthToSpace operator project directory.

  ```bash
  cd ${git_clone_path}/samples/operator_contrib/DepthToSpaceSample/FrameworkLaunch/DepthToSpace
  ```
...
- Execute the following command in the DepthToSpace operator project directory to compile the operator project.
...
- [Invoke the DepthToSpace Operator Project Using aclnn](./AclNNInvocation/README.en.md)
