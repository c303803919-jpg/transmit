...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnDepthToSpaceGetWorkspaceSize(inputTensor_[0], opDesc_->block_size, opDesc_->mode,
                                                opDesc_->dataFormat, outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnDepthToSpace(workspace, workspaceSize, handle, stream);
