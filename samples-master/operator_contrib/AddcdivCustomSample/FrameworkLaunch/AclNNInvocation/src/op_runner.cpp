...
*/
#include "op_runner.h"
#include "aclnn_addcdiv_custom.h"
#include <limits>
#include <cassert>
...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnAddcdivCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], float(1.0), outputTensor_[0],
	...
	INFO_LOG("Execute aclnnAddcdivCustomGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    if ((ret = aclnnAddcdivCustom(workspace, workspaceSize, handle, stream)) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnAddcdivCustom success");
