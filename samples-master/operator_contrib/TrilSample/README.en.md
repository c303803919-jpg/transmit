## Tril Custom Operator Example Explanation
This example implements the Tril operator using the Ascend C programming language and provides end-to-end implementations for different operator invocation methods.

- [FrameworkLaunch](./FrameworkLaunch/README.en.md): Invokes the Tril custom operator using the framework.
...
<table>
    ...
    <tr>
        ...
        <td rowspan='1'><a href="./FrameworkLaunch/README.en.md"> FrameworkLaunch</td><td><a href="./FrameworkLaunch/AclNNInvocation/README.en.md"> AclNNInvocation</td><td>Invokes the Tril operator using the aclnn method.</td>
    </tr>
</table>

## Operator Description
The Tril operator is a common matrix constructor used in PyTorch. It, by default, calculates the lower triangular matrix below the principle diagonal of the given matrix, with all other elements set to 0. The offset of the principle diagonal is determined by the optional parameter diagonal, default to 0. The principle diagonal would move up if diagonal contains a positive value. When the input is a multi-dimensional tensor, the very last two dimensions of the tensor would be considered as matrices. Tril would iterate over all these matrices in the input tensor, and returns a tensor consisting of the corresponding lower triangular matrices. The returned tensor keeps its shape with the input tensor.  

## Operator Specification Description
<table>
<tr><th align="center">Operator Type(OpType)</th><th colspan="5" align="center">Tril</th></tr>
...
<tr><td rowspan="1" align="center">attr attribute</td><td align="center">diagonal</td><td align="center">\</td><td align="center">int</td><td align="center">\</td><td align="center">0</td></tr>
<tr><td rowspan="1" align="center">Kernel Function Name</td><td colspan="5" align="center">tril</td></td></tr>
</table>
...
```
└── FrameworkLaunch    // Project for invoking the Tril custom operator using the framework.
```
