## 概述
本样例基于Tril算子工程，介绍了单算子调用的调用方式。
## 目录结构介绍
``` 
├── FrameworkLaunch           //使用框架调用的方式调用Tril算子
│   ├── AclNNInvocation       // 通过aclnn调用的方式调用Tril算子
│   ├── Tril             // Tril算子工程
│   └── Tril.json        // Tril算子的原型定义json文件
``` 
## 算子工程介绍
算子工程目录Tril包含算子实现的模板文件、编译脚本等，如下所示:
``` 
├── Tril               //Tril自定义算子工程
│   ├── cmake
...
### 4.&nbsp;配置环境变量
  ...
  - 指定路径install_path，安装CANN软件包
    ...
    ```
### 5.&nbsp;调用执行算子工程
- [aclnn调用Tril算子工程](./AclNNInvocation/README.md)
