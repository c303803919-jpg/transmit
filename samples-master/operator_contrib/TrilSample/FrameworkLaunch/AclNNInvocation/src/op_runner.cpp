...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnTrilGetWorkspaceSize(inputTensor_[0], opDesc_->diagonal, outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnTril(workspace, workspaceSize, handle, stream);
