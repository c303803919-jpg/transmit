## Overview
This example, based on the Tril operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Tril operator using the framework
│   ├── AclNNInvocation       // Invokes the Tril operator using the aclnn method
│   ├── Tril             // Tril operator project
│   └── Tril.json        // Prototype definition json file for the Tril operator
``` 

## Operator Project Introduction
The operator project directory Tril contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── Tril               // Tril custom operator project
│   ├── cmake
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Tril operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/TrilSample/FrameworkLaunch/Tril
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Tril operator project directory to compile the operator project.
...
- [Invoke the Tril Operator Project using aclnn](./AclNNInvocation/README.en.md)
