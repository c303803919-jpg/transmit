...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnLerpGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnLerp(workspace, workspaceSize, handle, stream);
