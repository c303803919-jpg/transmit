## Overview
This example, based on the Lerp operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Lerp operator using the framework
│   ├── AclNNInvocation       // Invokes the Lerp operator using the aclnn method
│   ├── Lerp             // Lerp operator project
│   └── Lerp.json        // Prototype definition json file for the Lerp operator
```

## Operator Project Introduction
The operator project directory Lerp contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── Lerp               // Lerp custom operator project
│   ├── cmake
...
```
The CANN software package provides the project creation tool msopgen, and the Lerp operator project can be automatically created through Lerp.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Lerp operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/LerpSample/FrameworkLaunch/Lerp
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Lerp operator project directory to compile the operator project.
...
- [Invoke the Lerp Operator Project using aclnn](./AclNNInvocation/README.en.md)
