...
bool OpRunner::RunOp()
{
    ...
    string reduction;
	auto ret = aclnnScatterSubGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2],opDesc_->use_locking,
    ...
    ret = aclnnScatterSub(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnScatterSub success");
