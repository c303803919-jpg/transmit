...
class FlashAttentionScoreTiling {
public:
    explicit FlashAttentionScoreTiling(gert::TilingContext *context) : context_(context)
    {
        context_ = context;
        tilingData.SetDataPtr(context_->GetRawTilingData()->GetData());
    }
    ~FlashAttentionScoreTiling() = default;
...
protected:
    ...
    const char *templateName = "FlashAttentionScoreS1s2Bn2gs1";
    int64_t s2sizeLimitMin = 1024;
    FlashAttentionScoreTilingData tilingData;
};

ge::graphStatus FlashAttentionScoreTiling::CheckContext()
{
...
ge::graphStatus FlashAttentionScoreTiling::DoOpTiling()
{
...
bool FlashAttentionScoreTiling::SetBmm1TilingInput(int64_t s1BasicBlock, int64_t s2BasicBlock,
                                                       matmul_tiling::MatmulApiTiling &bmm1)
{
...
bool FlashAttentionScoreTiling::SetBmm2TilingInput(int64_t s1BasicBlock, int64_t s2BasicBlock, int64_t dBasicBlock,
                        matmul_tiling::MatmulApiTiling &bmm2)
{
...
bool FlashAttentionScoreTiling::SetMatMulTiling(int64_t s1BasicBlock, int64_t s2BasicBlock,
                                                    int64_t dBasicBlock,
                                                    matmul_tiling::MatmulApiTiling &bmm1,
                                                    matmul_tiling::MatmulApiTiling &bmm2)
{
...
bool FlashAttentionScoreTiling::IsBasicBlockInSoftMax(const ge::Shape &shape) const
{
...
ge::graphStatus FlashAttentionScoreTiling::DoLibApiTiling()
{
