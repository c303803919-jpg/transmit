## Overview
This example, based on the Eye operator project, introduces the invocation method for single operator calls.

## Directory Structure Introduction
``` 
├── FrameworkLaunch           // Invokes the Eye operator using the framework
│   ├── AclNNInvocation       // Invokes the Eye operator using the aclnn method
│   ├── Eye             // Eye operator project
│   └── Eye.json        // Prototype definition json file for the Eye operator
```

## Operator Project Introduction
The operator project directory Eye contains template files for operator implementation, compilation scripts, etc., as shown below:
``` 
├── Eye               // Eye custom operator project
│   ├── cmake
...
```
The CANN software package provides the project creation tool msopgen, and the Eye operator project can be automatically created through Eye.json. For details, please refer to the [Ascend C Operator Development](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC) > Operator Development > Operator Development Project > Operator Development Based on Custom Operator Project > Creating Operator Project section.
...
Compile the custom operator project to build and generate the custom operator package.
  ...
  - Execute the following command to switch to the Eye operator project directory.

    ```bash
    cd ${git_clone_path}/samples/operator_contrib/EyeSample/FrameworkLaunch/Eye
    ```
  ...
  - Modify the ASCEND_CANN_PACKAGE_PATH in CMakePresets.json to the actual path after the CANN software package is installed.
    ...
    ```
  - Execute the following command in the Eye operator project directory to compile the operator project.
...
- [Invoke the Eye Operator Project using aclnn](./AclNNInvocation/README.en.md)
