...
bool OpRunner::RunOp()
{
    ...
    aclIntArray *batchShape = nullptr;
    if (opDesc_->batchShape.size() > 0)
    {
        batchShape = aclCreateIntArray(opDesc_->batchShape.data(), opDesc_->batchShape.size());
    }
    
	auto ret = aclnnEyeGetWorkspaceSize(inputTensor_[0],opDesc_->num_rows, opDesc_->num_columns, 
                                        batchShape,opDesc_->dtype, &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnEye(workspace, workspaceSize, handle, stream);
