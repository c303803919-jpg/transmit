...
*/
#include "op_runner.h"
#include "aclnn_lp_norm_v2_custom.h"
#include <limits>
#include <cassert>
...
bool OpRunner::RunOp()
{
    ...
    auto ret = aclnnLpNormV2CustomGetWorkspaceSize(inputTensor_[0], opDesc_->pOptional, axes, opDesc_->keepdimOptional, opDesc_->epsilonOptional, outputTensor_[0],
	...
	INFO_LOG("Execute aclnnLpNormV2CustomGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    if (aclnnLpNormV2Custom(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnLpNormV2Custom success");
