...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnReduceSumGetWorkspaceSize(inputTensor_[0], inputTensor_[1], opDesc_->keepDims, opDesc_->ignoreNan,
                                              opDesc_->dtype, outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
    ...
    ret = aclnnReduceSum(workspace, workspaceSize, handle, stream);
