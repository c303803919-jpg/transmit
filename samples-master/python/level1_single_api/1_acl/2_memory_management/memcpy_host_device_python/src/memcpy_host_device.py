...
def main():
    ...
    The units are Byte")
    parser.add_argument("--write_back_host", action='store_true', help="Whether to send back the host")
    ...
    logger.info("Using params are as follows.\n  device id : {}\n  release cycle : {}\n"
          "  number of cycles : {}\n  memory size : {} Bytes\n  write back host : {}\n  memory_reuse : {}\n"
          .format(args.device_id, args.release_cycle, args.number_of_cycles, args.memory_size, \
          args.write_back_host, args.memory_reuse))
    ...
    while (args.number_of_cycles-cycles_time != 0):
        ...
        if args.memory_reuse:
            ...
            if check_ret("acl.rt.malloc_host", ret):
                continue
            device_buffer, ret = acl.rt.malloc(args.memory_size, ACL_MEM_MALLOC_HUGE_FIRST)
            if check_ret("acl.rt.malloc", ret):
                continue
        ...
        while (args.release_cycle-release_cycle_time != 0):
            ...
            if copy_data_as_numpy("At memcpy device data", device_buffer, args.memory_size, ACL_MEMCPY_DEVICE_TO_HOST):
                continue
            # 判断是否要回传Host
            if args.write_back_host:
                # 申请回传所需的Host内存
                host_buffer, ret = acl.rt.malloc_host(args.memory_size)
                if check_ret("acl.rt.malloc_host", ret):
                    continue
                # 内存拷贝
                ret = acl.rt.memcpy(host_buffer, args.memory_size, device_buffer, args.memory_size, \
                ACL_MEMCPY_DEVICE_TO_HOST)
                if check_ret("acl.rt.memcpy", ret):
                    continue
                # 查看回传的数据
                if copy_data_as_numpy("Write back host data", host_buffer, args.memory_size, ACL_MEMCPY_HOST_TO_HOST):
                    continue
                # 回传所申请的Host内存单独释放
        ...
        for device_buffer in dev_mem_list:
            ret = acl.rt.free(device_buffer)
            if check_ret("acl.rt.free", ret):
                continue
        # 查询循环周期时的Device内存
        cycles_time += 1
        sleep(1)
    ...
    if ret != ACL_SUCCESS:
        logger.error("acl.rt.destroy_context failed ret = {}".format(ret))
        exit(1)
    context = None
    ret = acl.rt.reset_device(args.device_id)
    if ret != ACL_SUCCESS:
        logger.error("acl.rt.reset_device failed ret = {}".format(ret))
        exit(1)
    ret = acl.finalize()
    if ret != ACL_SUCCESS:
