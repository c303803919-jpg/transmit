...
dflow::FlowGraph BuildDataFlow()
{
    ...
    auto node0 = BuildFunctionNodeSimple(udf1BuildCfg, true)
                 .SetInput(0, data0)
                 .SetInput(1, data1);
...
int32_t main()
{
    ...
    const uint32_t alginMaxCacheNum = 100;
    const int32_t alignTimeout  = 1000;
    ...
    for (size_t i = 0; i < loopNum; ++i) {
        ...
        if (geRet != ge::SUCCESS) {
            std::cout << "ERROR=======Fetch data failed===========ret[" << geRet << "]" << std::endl;
            continue;
        }
