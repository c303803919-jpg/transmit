## 概述
本样例基于MatmulReduceScatterCustom算子工程，介绍了单算子工程、单算子调用调用。
## 目录结构介绍
```
├── 22_matmul_reduce_scatter_custom
│   ├── AclNNInvocation                       // 通过aclnn调用的方式调用MatmulReduceScatterCustom算子
│   ├── MatmulReduceScatterCustom             // MatmulReduceScatterCustom算子工程
│   ├── matmul_reduce_scatter_custom.json     // MatmulReduceScatterCustom算子的原型定义json文件
│   ├── matmul_reduce_scatter_demo_def.h      // MatmulReduceScatterCustom算子参数配置
│   └── install.sh                            // 脚本，调用msOpGen生成自定义算子工程，并编译
```

## 算子描述
MatmulReduceScatter算子实现了Matmul矩阵乘法运算操作和ReduceScatter通信操作的融合。

MatmulReduceScatter的数学表达式为：  
$$
...
备注：集合通信原语ReduceScatter请参考[集合通信用户指南](https://hiascend.com/document/redirect/CannCommunityHcclUg)>集合通信原语。


## 算子规格描述
<table>
<tr><td rowspan="1" align="center">算子类型(OpType)</td><td colspan="4" align="center">MatmulReduceScatterCustom</td></tr>
</tr>
...
<tr><td rowspan="2" align="center">算子输出</td><td align="center">y</td><td align="center">2048 * 5120</td><td align="center">float16</td><td align="center">ND</td></tr>

</tr>
<tr><td rowspan="1" align="center">核函数名</td><td colspan="4" align="center">matmul_reduce_scatter_custom</td></tr>
</table>
...
## 算子工程介绍
其中，算子工程目录MatmulReduceScatterCustom包含算子实现的模板文件、编译脚本等，如下所示：
```
├── MatmulReduceScatterCustom               //MatmulReduceScatter自定义算子工程
│   ├── cmake
│   ├── op_host             // host侧实现文件
...
```
CANN软件包中提供了工程创建工具msopgen，MatmulReduceScatterCustom算子工程可通过MatmulReduceScatterCustom.json自动创建，具体请参考[Ascend C自定义算子开发](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC)>工程化算子开发>创建算子工程 章节。

创建完自定义算子工程后，开发者重点需要完成算子host和kernel文件的功能开发。为简化样例运行流程，本样例已在MatmulReduceScatterCustom目录中准备好了必要的算子实现，install.sh脚本会创建一个CustomOp目录，并将算子实现文件复制到对应目录下，再编译算子。
...
### 2. 生成自定义算子工程，复制host和kernel实现并编译算子<a name="operatorcompile"></a>
  - 切换到msOpGen脚本install.sh所在目录
    ...
    cd ${git_clone_path}/samples/operator/ascendc/4_best_practices/22_matmul_reduce_scatter_custom
    ```
  ...
  - 调用脚本，生成自定义算子工程，复制host和kernel实现并编译算子
    ...
    备注：如果要使用dump调试功能，需要移除op_host内的Atlas A2训练系列产品的配置项。
...
### 4. 调用执行算子工程
- [aclnn调用MatmulReduceScatterCustom算子工程](./AclNNInvocation/README.md)
