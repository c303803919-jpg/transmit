...
#include "aclnn_matmul_reduce_scatter_custom.h"
...
bool OpRunner::RunOp(std::string group, aclrtStream stream)
{
    ...
    auto ret = aclnnMatmulReduceScatterCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], bias, (char*)group.c_str(),
    ...
    ret = aclnnMatmulReduceScatterCustom(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
