...
#include "../op_kernel/matmul_reduce_scatter_custom_tiling.h"
...
static ge::graphStatus MatmulReduceScatterCustomTilingFunc(gert::TilingContext *context) {
    // 对参数进行校验
    ...
    MatmulReduceScatterCustomTilingData *tiling = context->GetTilingData<MatmulReduceScatterCustomTilingData>();
...
namespace ops {
class MatmulReduceScatterCustom : public OpDef {
public:
    explicit MatmulReduceScatterCustom(const char *name) : OpDef(name)
    {
        this->Input("x1")
            .ParamType(REQUIRED)
        ...
        this->AICore().SetTiling(MatmulReduceScatterCustomTilingFunc);
        this->AICore().AddConfig("ascend910b", aicore_config);
        this->MC2().HcclGroup("group");
    }
};

OP_ADD(MatmulReduceScatterCustom);
}
