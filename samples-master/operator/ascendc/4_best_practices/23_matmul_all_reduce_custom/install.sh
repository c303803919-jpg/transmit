...
OP_NAME=MatmulAllReduceCustom
rm -rf CustomOp
# Generate the op framework
msopgen gen -i matmul_all_reduce_custom.json -c ai_core-${SOC_VERSION} -lan cpp -out CustomOp
rm -rf CustomOp/op_host/*tiling.h
# Copy op implementation files to CustomOp, select one of the following two options
cp -rf MatmulAllReduceCustom/* CustomOp # cp -rf MatmulCustomMultiCore/* CustomOp
