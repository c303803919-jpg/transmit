...
OP_NAME=AllGatherMatmulCustom
rm -rf CustomOp
# Generate the op framework
msopgen gen -i all_gather_matmul_custom.json -c ai_core-${SOC_VERSION} -lan cpp -out CustomOp
rm -rf CustomOp/op_host/*tiling.h
# Copy op implementation files to CustomOp, select one of the following two options
cp -rf AllGatherMatmulCustom/* CustomOp # cp -rf MatmulCustomMultiCore/* CustomOp
