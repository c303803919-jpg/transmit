...
private:
    __aicore__ inline void CopyIn(int32_t progress)
    {
        ...
        AscendC::DumpTensor(xGm[progress * this->tileLength], 0, DUMP_LENGTH);
        // 带Tensor shape的打印, 打印UB以及附加信息1
        uint32_t array[] = {static_cast<uint32_t>(8), static_cast<uint32_t>(8)};
        AscendC::ShapeInfo shapeInfo(2, array); // dim为2， shape为(8,8)
        AscendC::DumpTensor(xLocal, 1, 64, shapeInfo); // dump x的64个元素，且解析按照shapeInfo的(8,8)排列
        // 带偏移量的DumpAccChkPoint
        AscendC::DumpAccChkPoint(xLocal, 2, OFFSET_LENGTH, DUMP_LENGTH);  // 打印自定义附加信息2
