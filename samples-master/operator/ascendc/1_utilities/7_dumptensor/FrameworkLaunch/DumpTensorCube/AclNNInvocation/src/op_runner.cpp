...
bool OpRunner::RunOp()
{
    ...
    auto ret = aclnnMmadCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], outputTensor_[0], &workspaceSize,
        &handle);
    if (ret != ACL_SUCCESS) {
    ...
    INFO_LOG("Execute aclnnMmadCustomGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnMmadCustom(workspace_, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnMmadCustom success");
