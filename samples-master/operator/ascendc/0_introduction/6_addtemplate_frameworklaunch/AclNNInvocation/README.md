## 目录结构介绍
```
├── AclNNInvocation             //通过aclnn调用的方式调用AddTemplateCustom算子
│   ├── inc                     // 头文件目录
...
### 2. aclnn调用样例运行
  ...
  - 进入到样例目录   
    ...
    ```bash
    cd ${git_clone_path}/samples/operator/ascendc/0_introduction/6_addtemplate_frameworklaunch/AclNNInvocation
    ```
  - 样例执行
