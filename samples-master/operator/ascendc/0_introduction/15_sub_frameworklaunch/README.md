...
## 目录结构介绍
```
├── 15_sub_frameworklaunch    // 使用框架调用的方式调用SubCustom算子
│   ├── AclOfflineModel       // 通过aclopExecuteV2调用的方式调用SubCustom算子
│   ├── SubCustom             // SubCustom算子工程
...
## 算子工程介绍
其中，算子工程目录SubCustom包含算子的实现文件，如下所示:
```
├── SubCustom               // SubCustom自定义算子工程
│   ├── op_host             // host侧实现文件
│   └── op_kernel           // kernel侧实现文件
```
CANN软件包中提供了工程创建工具msOpGen，SubCustom算子工程可通过SubCustom.json自动创建，自定义算子工程具体请参考[Ascend C算子开发](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC)>工程化算子开发>创建算子工程 章节。

创建完自定义算子工程后，开发者重点需要完成算子host和kernel文件的功能开发。为简化样例运行流程，本样例已在SubCustom目录中准备好了必要的算子实现，install.sh脚本会创建一个CustomOp目录，并将算子实现文件复制到对应目录下，再编译算子。
...
### 2. 生成自定义算子工程，复制host和kernel实现并编译算子<a name="operatorcompile"></a>
  - 切换到msOpGen脚本install.sh所在目录
    ...
    cd ${git_clone_path}/samples/operator/ascendc/0_introduction/15_sub_frameworklaunch
    ```
...
### 样例运行
  - 切换到样例所在目录
    ```bash
    # 若开发者以git命令行方式clone了master分支代码，并切换目录
    cd ${git_clone_path}/samples/operator/ascendc/0_introduction/15_sub_frameworklaunch/AclOfflineModel
    ```
...
| 时间       | 更新事项                     |
| ---------- | -------------------- |
| 2023/11/09 | 新增AclOfflineModel样例 |
| 2024/11/11 | 样例目录调整 |
