...
bool OpRunner::Init()
{
    ...
    if (inputTensorList == nullptr) {
            ERROR_LOG("Create Tensor list for input failed");
            return false;
    }
...
bool OpRunner::RunOp()
{
    ...
    auto ret =
        aclnnAddnCustomGetWorkspaceSize(inputTensorList, outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Get Operator Workspace failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnAddnCustomGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnAddnCustom(workspace_, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnAddnCustom success");
