...
注意：若tensorflow版本是2.x，需注意插件代码适配，路径为： samples/operator/ascendc/0_introduction/1_add_frameworklaunch/AddCustom/framework/tf_plugin/tensorflow_add_custom_plugin.cc
...
### 3.tensorflow调用的方式调用样例运行
  ...
  - 进入到样例目录   
    ...
    ```bash
    cd ${git_clone_path}/samples/operator/ascendc/0_introduction/1_add_frameworklaunch/TensorflowInvocation/AscendCustomToTensorFlowBuildIn
    ```
