...
#include "aclnn_matmul_api_constant_custom.h"
#include "common.h"
...
bool OpRunner::RunOp()
{
    ...
    auto ret = aclnnMatmulApiConstantCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2], outputTensor_[0],
    ...
    INFO_LOG("Execute aclnnMatmulApiConstantCustomGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnMatmulApiConstantCustom(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnMatmulApiConstantCustomCustom success");
