...
#include "aclnn_cube_group_custom.h"
#include "common.h"
...
bool OpRunner::RunOp()
{
    ...
    auto ret =
        aclnnCubeGroupCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2],outputTensor_[0], &workspaceSize, &handle);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Get Operator Workspace failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnCubeGroupCustomGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnCubeGroupCustom(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
    INFO_LOG("Execute aclnnCubeGroupCustom success");
